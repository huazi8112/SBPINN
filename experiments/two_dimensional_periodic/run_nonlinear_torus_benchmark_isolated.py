...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run reference, SBPINN, each FEM mesh, and report in isolated "
            "processes."
        )
    )
...
def run_stage(
    ...
    if returncode == 0:
        ...
        print(
            f"{name} did not complete (exit code {returncode}); "
            f"continuing because it is an isolated FEM configuration."
        )
    return returncode
...
def main() -> None:
    ...
    runner = root / "run_nonlinear_torus_benchmark.py"
    ...
    sbpinn_done = all(
        ...
        for path in (
            ...
            actual_output / "sbpinn/sbpinn_checkpoint.pt",
            actual_output / "sbpinn/sbpinn_evaluation.npz",
        )
    )
    ...
    if not (args.resume and reference_done):
        ...
        print("Skipping completed reference stage.")
    ...
    if not (args.resume and sbpinn_done):
        ...
        print("Skipping completed SBPINN stage.")
    ...
    for mesh in meshes + precheck:
        result_path = (
            actual_output
            / f"fem/fem_n{mesh}_result.json"
        )
        if args.resume and result_path.exists():
            print(f"Skipping completed FEM mesh {mesh}.")
            continue
        returncode = run_stage(
            ...
            env,
            timeout=args.fem_time_budget_s + 120.0,
            allow_failure=True,
        )
