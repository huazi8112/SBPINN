...
def fractional_derivative_GL_asymptotic(func_log_p, x, t, alpha, dx=0.01, padding=300, limit=12.0,
                                          central_chunk=64, query_chunk=32768):
    """Mathematically identical standard/unshifted GL evaluation, but memory-safe.

    The original implementation materialized all 5000 x 1301 x 2 auxiliary
    queries at once.  This version evaluates the same stencil in chunks and
    accumulates the same weighted GL sums, without changing the stencil,
    coefficients, padding law, network, random seeds, or optimization logic.
    """
    ...
    # Keep the ORIGINAL paper code's standard/unshifted GL exactly:
    # x-k*dx and x+k*dx.
    ...
    def eval_log_p_queries(x_query, t_query):
        """Same interior NN / exterior asymptotic rule as the original, chunked."""
        vals_all = []
        for qs in range(0, x_query.shape[0], query_chunk):
            xq = x_query[qs:qs + query_chunk]
            tq = t_query[qs:qs + query_chunk]
            mask_inside = (xq.abs() <= limit)
            log_p_vals = torch.zeros_like(xq)

            if mask_inside.any():
                x_in = xq[mask_inside].view(-1, 1)
                t_in = tq[mask_inside].view(-1, 1)
                log_p_vals[mask_inside] = func_log_p(x_in, t_in).squeeze()

            if (~mask_inside).any():
                x_out = xq[~mask_inside].view(-1, 1)
                t_out = tq[~mask_inside].view(-1, 1)
            ...
            vals_all.append(torch.exp(torch.clamp(log_p_vals, max=20.0)))
        return torch.cat(vals_all, dim=0)

    # Evaluate the identical GL stencil for small blocks of central points.
    for bs in range(0, batch_size, central_chunk):
        xb = x_flat[bs:bs + central_chunk]
        tb = t_flat[bs:bs + central_chunk]
        B = xb.shape[0]

        x_left = xb - shifts
        x_right = xb + shifts
        t_rep = tb.repeat_interleave(len(w_tensor), dim=0)

        p_left = eval_log_p_queries(x_left.flatten().view(-1, 1), t_rep).view(B, -1)
        p_right = eval_log_p_queries(x_right.flatten().view(-1, 1), t_rep).view(B, -1)

        gl_sum = (p_left * w_tensor).sum(dim=1, keepdim=True) + \
                 (p_right * w_tensor).sum(dim=1, keepdim=True)
        outputs.append(coef * gl_sum)
