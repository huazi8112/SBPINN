...
def run_experiment(target_alpha):
    ...
    # === 绘图 ===
    print(f"正在生成结果图 (Alpha={target_alpha})...")
    ...
    # Loss 曲线
    x_axis = np.arange(len(ls_L)) * 10
    ax_ls = fig.add_subplot(gs[0, 0])
    ax_ls.plot(x_axis, smooth_curve(ls_L, 0.95), 'b', alpha=0.8, label='Expert L')
    ax_ls.plot(x_axis, smooth_curve(ls_R, 0.95), 'g', alpha=0.8, label='Expert R')
    ax_ls.set_title(f"Score Loss (Alpha={target_alpha})")
    ax_ls.legend()
    ax_ls.grid(alpha=0.3)

    ax_ll = fig.add_subplot(gs[0, 1])
    ax_ll.plot(x_axis, smooth_curve(ll_L_hist, 0.95), 'b', alpha=0.8, label='Expert L')
    ax_ll.plot(x_axis, smooth_curve(ll_R_hist, 0.95), 'g', alpha=0.8, label='Expert R')
    ax_ll.set_title(f"LL Loss (Alpha={target_alpha})")
    ...
    for idx, t in enumerate(t_evals):
        ...
        ax.hist(mc_data, bins=80, density=True, color='lightgray',
                edgecolor='black', alpha=0.7, label='Monte Carlo')
        ...
        ax.plot(x_grid, p_total, 'r-', linewidth=2.5, label='Ours')
        ax.plot(x_grid, 0.5 * p_L, 'b--', linewidth=1, alpha=0.5)
        ax.plot(x_grid, 0.5 * p_R, 'g--', linewidth=1, alpha=0.5)
        ax.text(0.03, 0.92, f"W-Dist: {w_dist:.4f}", transform=ax.transAxes,
    ...
    plt.suptitle(f"Bistable ($\\alpha={target_alpha}$)", fontsize=14,
                 y=0.95)  # 顺便帮你把标题里的 Alpha 改成了 LaTeX 公式格式 $\alpha$

    fname_png = f"bimodal_{target_alpha}.png"
    fname_pdf = f"bimodal_{target_alpha}.pdf"

    # 保存 PNG (增加 dpi 参数保证清晰度)
